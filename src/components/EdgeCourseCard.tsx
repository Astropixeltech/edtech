        ...
        <div className="space-y-2.5">
          ...
          {course.instructor_name && (
            <p className="text-xs text-slate-700 dark:text-slate-300 truncate">
              ...
              <span className="font-bold text-slate-900 dark:text-white">{course.instructor_name}</span>
            </p>
          )}
          ...
          <div className="flex items-center gap-3 text-[11px] text-slate-700 dark:text-slate-300 font-bold pt-1">
            {course.total_enrolled !== undefined && (
              <span className="flex items-center gap-1">
                <Users className="h-3.5 w-3.5 text-primary dark:text-emerald-400" />
                <span>{course.total_enrolled} {isBn ? "জন" : "students"}</span>
              </span>
            )}
            {course.duration && (
              <span className="flex items-center gap-1">
                <Clock className="h-3.5 w-3.5 text-primary dark:text-emerald-400" />
          ...
          <div className="flex flex-col min-w-0">
            ...
            <div className="flex items-baseline gap-1.5 flex-wrap">
              ...
              {course.sale_price !== undefined && course.sale_price !== null && course.price && course.price > course.sale_price && (
                <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold line-through">
